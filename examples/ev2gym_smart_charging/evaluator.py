"""EV2Gym smart charging evaluator for agentic-evolve (Frontier subprocess bridge)."""
...
def _resolve_paths() -> tuple[Path, Path, Path]:
    ...
    benchmark_dir = frontier_root / "benchmarks" / "PowerSystems" / "EV2GymSmartCharging"
    ...
    python_bin = Path(
        os.environ.get(
            ...
            os.environ.get(
                ...
                str(frontier_root / ".venvs" / "frontier-eval-driver" / "bin" / "python"),
            ),
        )
    ).expanduser()
...
def evaluate(program_path: str, output_dir: str) -> dict:
    ...
    if not python_bin.exists():
        return _failure_result(
            f"EV2Gym python not found: {python_bin}. "
            "Run Frontier-Engineering setup_v1_task_envs.sh or set EV2GYM_PYTHON."
        )
    ...
    score = float(report.get("score", metrics_payload.get("score", 0.0)))
    metrics = {
        key: float(value)
