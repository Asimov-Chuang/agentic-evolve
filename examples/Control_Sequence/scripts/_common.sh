#!/usr/bin/env bash
# Shared helpers for feedback-noise ablation replicate runs.
set -euo pipefail
...
REPLICATE_START="${REPLICATE_START:-5}"
REPLICATE_END="${REPLICATE_END:-8}"

# CloudGPT proxy (optional). Set USE_CLOUDGPT=1 to route OpenCode through the local proxy.
...
run_replicates() {
  ...
  local i project_name tmp_config workspace_hint config_stem
