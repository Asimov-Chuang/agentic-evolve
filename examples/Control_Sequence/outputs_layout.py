"""Helpers for feedback-noise ablation output directory layout."""
...
# Internal setting labels (for analysis scripts).
SETTING_ORDER = ("score_only", "0pct", "50pct", "80pct", "0pct_metric_meanings")
LEGACY_SETTING_DIRS = frozenset(SETTING_ORDER)

# Blind names visible to the agent in workspace paths (no noise-ratio hints).
BLIND_GROUP_TO_SETTING: dict[str, str] = {
    "cond_a": "score_only",
    "cond_b": "0pct",
    "cond_c": "50pct",
    "cond_d": "80pct",
    "cond_e": "0pct_metric_meanings",
}
...
_LEGACY_REPLICATE_PAT = re.compile(
    r"^feedback_ablation_(?:(score_only)_(\d+)|(0pct|50pct|80pct|0pct_metric_meanings)_(\d+))$"
)
...
def parse_project_dir(name: str) -> tuple[str, int] | None:
    """Parse legacy project dir names that embed the setting (e.g. feedback_ablation_0pct_3)."""
...
def parse_blind_project_dir(group: str, name: str) -> tuple[str, int] | None:
    """Parse blind layout: cond_b/ablation_run_3 -> ('0pct', 3)."""
...
def iter_project_dirs(outputs_root: Path) -> Iterator[tuple[str, Path]]:
    """Yield (setting, workspace_dir) under blind, legacy nested, and flat layouts."""
