"""general_meio evaluator for agentic-evolve (Frontier-Engineering subprocess bridge)."""
...
def _python_has_stockpyl(python_bin: Path) -> bool:
    try:
        completed = subprocess.run(
            [str(python_bin), "-c", "import stockpyl"],
            capture_output=True,
            timeout=15,
        )
...
def _resolve_python_bin(frontier_root: Path) -> Path | None:
    env_python = os.environ.get("GENERAL_MEIO_PYTHON", "").strip()
    ...
    for candidate in candidates:
        ...
        if _python_has_stockpyl(python_bin):
            return python_bin
    return None
...
def _resolve_paths() -> tuple[Path, Path, Path]:
    ...
    benchmark_dir = frontier_root / "benchmarks" / "InventoryOptimization" / "general_meio"
    evaluate_py = benchmark_dir / "verification" / "evaluate.py"
    python_bin = _resolve_python_bin(frontier_root)
    if python_bin is None:
        raise FileNotFoundError(
            "No Python interpreter with stockpyl found. "
            "Run Frontier-Engineering setup_v1_task_envs.sh or set GENERAL_MEIO_PYTHON."
        )
...
def evaluate(program_path: str, output_dir: str) -> dict:
    ...
    try:
        _, evaluate_py, python_bin = _resolve_paths()
    ...
    score = float(report.get("final_score", metrics_payload.get("final_score", 0.0)))
    metrics = {
        key: float(value)
        for key, value in (report.get("metrics") or metrics_payload).items()
