"""Workspace analyzer delegate; calls private analyzer logic via subprocess."""
...
ENV_ROOT = "CAUSAL_FILTER_ABLATION_EXAMPLE_ROOT"
...
def _example_root() -> Path:
    ...
    for candidate in (here, *here.parents):
        if (candidate / "evaluator_core.py").is_file():
            return candidate
    raise RuntimeError("Cannot locate Causal_Filter_Ablation example root")
...
def _run_analyze(program_path: str, result: dict, noise_ratio: float) -> dict:
