"""HighReliableSimulation evaluator for agentic-evolve (Frontier subprocess bridge)."""
...
def _python_has_runtime(python_bin: Path) -> bool:
    try:
        completed = subprocess.run(
            [str(python_bin), "-c", "import numpy, scipy"],
            capture_output=True,
            timeout=120,
        )
...
def _resolve_python_bin(frontier_root: Path) -> Path | None:
    env_python = os.environ.get("HIGH_RELIABLE_SIM_PYTHON", "").strip() or os.environ.get(
        "FRONTIER_EVAL_DRIVER_PYTHON", ""
    ).strip()
    candidates: list[Path] = []
    if env_python:
        candidates.append(Path(env_python).expanduser())
    candidates.append(frontier_root / ".venvs" / "frontier-eval-driver" / "bin" / "python")
    ...
    for candidate in candidates:
        ...
        if _python_has_runtime(python_bin):
            return python_bin
    return None
...
def _resolve_paths() -> tuple[Path, Path, Path]:
    ...
    if python_bin is None:
        raise FileNotFoundError(
            "No Python interpreter with numpy/scipy found. "
            "Run Frontier-Engineering init.sh or set HIGH_RELIABLE_SIM_PYTHON / "
            "FRONTIER_EVAL_DRIVER_PYTHON."
        )
...
def evaluate(program_path: str, output_dir: str) -> dict:
    ...
    env = os.environ.copy()
    env["FRONTIER_ENGINEERING_ROOT"] = str(frontier_root)
    ...
    score = float(metrics_payload.get("combined_score", report.get("combined_score", -1e18)))
    valid = float(metrics_payload.get("valid", report.get("valid", 0.0))) > 0.0
    metrics = {
    ...
    return {
        "score": score,
        "is_valid": valid or score > -1e17,
        "feedback": f"Score: {score:.6g} (valid={int(valid)})",
        "metrics": metrics,
        "construction": report,
    }
