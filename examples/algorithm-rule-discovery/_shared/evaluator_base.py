"""Shared algorithm-based rule discovery evaluator logic."""
...
CACHE_FILENAME = "dataset_cache_code.pkl"
...
def _validate_rule_function_body(func: ast.FunctionDef) -> str | None:
    for node in ast.walk(func):
        if isinstance(node, ast.Name) and node.id in FORBIDDEN_NAMES:
            return (
                f"Rule {func.name!r} must not reference {node.id!r}. "
                "Analyze only the code string argument."
            )
        if isinstance(node, ast.Call):
            ...
            if call_name in {"open", "exec", "eval", "__import__"}:
                return f"Rule {func.name!r} must not call {call_name}()"
    return None


def _validate_program_reads_code_only(program_path: str) -> str | None:
    ...
    for node in tree.body:
        if isinstance(node, ast.FunctionDef) and node.name.startswith("rule_"):
            checked = True
            err = _validate_rule_function_body(node)
            if err:
                return err
    if not checked:
        return "Program must define at least one rule_* function"
    return None
...
) -> Tuple[List[Callable[[str], Tuple[int, str]]], List[str], List[str]]:
...
def _load_code_dataset_cache(
    ...
    for attempt_dir in sorted(dataset_dir.glob("attempt_*")):
        ...
        samples.append((attempt_dir.name, code_text, float(score)))
...
def _apply_rules(
    rules: List[Callable[[str], Tuple[int, str]]],
    samples: List[Tuple[str, str, float]],
    ...
    for attempt_id, code_text, score in samples:
        row: List[int] = []
...
def evaluate_core_code(
    ...
    forbidden = _validate_program_reads_code_only(program_path)
    if forbidden:
        return _failure_result(forbidden)
    try:
        rules, rule_names, rule_descriptions = _load_rules_and_descriptions(
            program_path, workspace=workspace
        )
    except Exception as exc:
    ...
    try:
        samples = _load_code_dataset_cache(dataset_dir, cache_path)
    except Exception as exc:
        return _failure_result(f"Failed to load dataset: {exc}")

    if not samples:
        return _failure_result("No samples with code.py and result.json score found")
...
try:
    spec = importlib.util.spec_from_file_location("ard_evaluator", evaluator_path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
